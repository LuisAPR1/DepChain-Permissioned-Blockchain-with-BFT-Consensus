...
public class Depchain {
	...
	public static void main(String[] args)
		...
		if (args.length < 3) {
			System.err.print("Usage: java <class_path> <replicaID> <configFilePath> <thresholdParamsPath>");
			System.exit(1);
		}

		int replicaID = Integer.parseInt(args[0]);
		String configPath = args[1];
		String thresholdParamsPath = args[2];
		...
		// Load shared ThresholdCrypto parameters (pre-generated by dealer)
		int f = (numReplicas - 1) / 3;
		int threshold = numReplicas - f;  // quorum size (2f + 1)
		ThresholdCrypto.DealerParams thresholdParams;
		try {
			thresholdParams = ThresholdCrypto.DealerParams.loadFromFile(thresholdParamsPath);
			System.out.println("[Depchain] Loaded shared threshold crypto params from " + thresholdParamsPath);
		} catch (ClassNotFoundException e) {
			throw new RuntimeException("Failed to load threshold params from " + thresholdParamsPath, e);
		}
		...
		int basePort = members[replicaID].getNetAddress().getPort();
		hotStuff = new HotStuff(replicaID, ownAddress, "localhost", basePort, numReplicas, ownKey, publicKeys, crypto, thresholdCrypto);
